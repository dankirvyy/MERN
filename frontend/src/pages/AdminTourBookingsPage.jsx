...
const AdminTourBookingsPage = () => {
    ...
    const handleMarkPaid = async (bookingId) => {
        if (!confirm('Mark this tour booking as fully paid?')) return;
        ...
        try {
            ...
            await axios.put(
                `http://localhost:5001/api/admin/tour-bookings/${bookingId}/mark-paid`,
                {},
                config
            );

            alert('Tour booking marked as paid');
            fetchBookings();
        } catch (error) {
    ...
    const handleRefund = async (bookingId) => {
        if (!confirm('Process refund for this cancelled tour booking? A confirmation email will be sent to the guest.')) return;
        ...
        try {
            ...
            await axios.put(
                `http://localhost:5001/api/admin/tour-bookings/${bookingId}/refund`,
                {},
                config
            );
